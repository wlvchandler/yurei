...
namespace oni16 {
    Options oni16_options;
    void parseOptions(const std::vector<std::string>& args) {
        if (args.empty()) {
            throw std::runtime_error("Usage: oni16 <input_file> [-p]");
        }

        for (size_t i = 0; i < args.size(); ++i) {
            const auto& arg = args[i];
            if (arg[0] != '-' && oni16_options.inputFile.empty()) {
                ...
                oni16_options.inputFile = filePath.string();
            }
            else if (arg == "-p") {
                oni16_options.printbytes = true;
            }
            //else if (arg == "-x") {
            //    oni16_options.littleEndian = true;
            //}
            //else if (arg == "-X") {
            //    oni16_options.bigEndian = true;
            //}
            //else if (arg == "-v") {
            //    oni16_options.verbose = true;
            //}
            else {
        ...
        if (oni16_options.inputFile.empty()) {
    ...
    bool Assembler::validateOperands() {
        ...
        if (expected == expectedOperands.end()) {
            throw std::runtime_error("oni16 error: unknown mnemonic: `" + mnemonic + "`");
        }
        ...
        if (op1.type == TokenType::Label) {
            ...
            if (label_it == symbolTable.end()) {
                throw std::runtime_error("oni16 error: unknown label: `" + label + "`");
            }
        ...
        throw std::runtime_error("oni16 error: invalid operands for `" + mnemonic + "`");
    }
    ...
    static void _generate(const M& map, const std::string& k, std::vector<uint16_t>& output, uint16_t mask = 0) {
        auto it = map.find(k);
        if (it == map.end()) {
            throw std::runtime_error("oni16 error: parse: unknown or invalid token: `" + k + "`");
        }
            ...
            if (instruction.mnemonic.type == TokenType::Opcode && validateOperands()) {
                ...
                for (auto& operand : instruction.operands) {
                    ...
                    case TokenType::Label:
                        ...
                        break;
                    default:
                        throw std::runtime_error("oni16 error: unknown operand type for " + std::get<std::string>(instruction.mnemonic.value));
                    };
                }
            }
        }
    ...
    void Assembler::assemble() {
        const std::string& f(oni16_options.inputFile);
        std::ifstream file(f);
        if (!file.is_open()) {
            throw std::runtime_error("oni16 error: failed to open file " + f);
        }
        ...
        writeBinary(filePath.string() + "/out.o16");


        if (oni16_options.printbytes) {
            int i = 0;
